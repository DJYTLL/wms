<template>
  <div class="page-shell page-shell--system">
    <div class="page-header">
      <div class="page-title">{{ $t('page.erpPaymentMethodManagement') }}</div>
      <div class="page-toolbar-card">
        <div class="erp-basic-toolbar">
          ...
          <div class="erp-basic-actions">
            <el-button type="primary" v-permission="'erp-payment-method:add'" @click="openAddModal">{{ $t('action.add') }}</el-button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="table-body">
        <el-table :data="tableData" style="width: 100%" stripe v-loading="loading" :empty-text="$t('table.empty')">
          ...
          <el-table-column :label="$t('table.actions')" width="160" fixed="right">
            <template #default="{ row }">
              <el-button link type="primary" size="small" v-permission="'erp-payment-method:edit'" @click="openEditModal(row)">{{ $t('action.edit') }}</el-button>
              <el-button link type="danger" size="small" v-permission="'erp-payment-method:delete'" @click="handleDelete(row)">{{ $t('action.delete') }}</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
const { isVisible, fetchTenantKeys } = useColumnSettings('erp-payment-method', defaultColumns);
...
const fetchList = async () => {
  ...
  try {
    ...
    const res: any = await request.get('/erp/payment-methods/page', { params });
...
const saveData = async () => {
  ...
  try {
    ...
    const res: any = isEditing.value && currentId.value
      ? await request.put(`/erp/payment-methods/${currentId.value}`, payload)
      : await request.post('/erp/payment-methods', payload);
...
const handleDelete = async (row: PaymentMethod) => {
  try {
    await request.delete(`/erp/payment-methods/${row.id}`);
    notifySuccess();
    fetchList();
  } catch (error) {
    notifyError(error);
