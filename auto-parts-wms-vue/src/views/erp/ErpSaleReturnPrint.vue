<template>
  <div class="print-page">
    <div v-if="!isPreview" class="print-toolbar">
      <div class="toolbar-title">{{ $t('page.erpSaleReturnPrint') }}</div>
...
const headerTitle = computed(() => template.value?.headerTitle || t('print.saleReturnTitle'));
...
const headerFieldLabel = (key: string) => {
  const mapping: Record<string, string> = {
    ...
    saleOrderNo: t('field.saleOrderNo'),
...
const formatReturnSource = (value?: string) => {
  if (value === 'BY_SALE_ORDER') return t('returnSource.bySaleOrder');
...
const getCustomerName = (id?: number) => customers.value.find((item) => item.id === id)?.name || '-';
...
const fetchSaleOrderNo = async (id?: number) => {
  ...
  try {
    ...
    saleOrderNo.value = data.order?.orderNo || data.orderNo || '';
  } catch {
    saleOrderNo.value = '';
  }
};
...
const fetchTemplate = async () => {
  try {
    const res: any = await request.get('/erp/print-templates/default', { params: { docType: 'SALE_RETURN' } });
