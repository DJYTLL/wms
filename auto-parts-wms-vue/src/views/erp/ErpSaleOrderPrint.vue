<template>
  <div class="print-page">
    <div v-if="!isPreview" class="print-toolbar">
      <div class="toolbar-title">{{ $t('page.erpSaleOrderPrint') }}</div>
...
const headerFieldLabel = (key: string) => {
  const mapping: Record<string, string> = {
    ...
    deliveryMethod: t('field.deliveryMethod'),
...
const headerFieldValue = (key: string) => {
  ...
  switch (key) {
    ...
    case 'deliveryMethod':
      return mapMethodName(deliveryMethods.value, current.deliveryMethod);
    case 'paidAmount':
...
const fetchTemplate = async () => {
  try {
    const res: any = await request.get('/erp/print-templates/default', { params: { docType: 'SALE_ORDER' } });
