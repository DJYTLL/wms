<template>
  <div class="print-page">
    <div v-if="!isPreview" class="print-toolbar">
      <div class="toolbar-title">{{ $t('page.erpAccountsPayablePrint') }}</div>
    ...
    <div class="print-paper" :class="{ 'print-paper--loading': loading }">
      ...
      <div v-if="showTotals" class="paper-totals">
        ...
        <div class="totals-item">
          ...
          <strong>{{ formatMoney(payable?.discountAmount) }}</strong>
        </div>
      </div>
...
const payableId = computed(() => Number(route.params.id));
...
const headerTitle = computed(() => template.value?.headerTitle || t('print.apTitle'));
...
const fetchTemplate = async () => {
  try {
    ...
    template.value = await fetchPrintTemplate('ACCOUNTS_PAYABLE', resolveTemplateId(route.query.templateId));
    applyTemplate(template.value);
  } catch {
    template.value = null;
    applyTemplate();
...
const fetchDetail = async () => {
  const res: any = await request.get(`/erp/ap/${payableId.value}`);
  const data = res.data.data || {};
  payable.value = data.payable || null;
...
const recordPrint = async () => {
  try {
    await request.post('/erp/print/logs', {
      docType: 'ACCOUNTS_PAYABLE',
      docId: payableId.value,
