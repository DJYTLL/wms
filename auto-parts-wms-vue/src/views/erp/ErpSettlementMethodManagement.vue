<template>
  <div class="page-shell page-shell--system">
    <div class="page-header">
      <div class="page-title">{{ $t('page.erpSettlementMethodManagement') }}</div>
      <div class="page-toolbar-card">
        <div class="erp-basic-toolbar">
          ...
          <div class="erp-basic-actions">
            <el-button type="primary" v-permission="'erp-settlement-method:add'" @click="openAddModal">{{ $t('action.add') }}</el-button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="table-body">
        <el-table :data="tableData" style="width: 100%" stripe v-loading="loading" :empty-text="$t('table.empty')">
          ...
          <el-table-column :label="$t('table.actions')" width="160" fixed="right">
            <template #default="{ row }">
              <el-button link type="primary" size="small" v-permission="'erp-settlement-method:edit'" @click="openEditModal(row)">{{ $t('action.edit') }}</el-button>
              <el-button link type="danger" size="small" v-permission="'erp-settlement-method:delete'" @click="handleDelete(row)">{{ $t('action.delete') }}</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
const fetchList = async () => {
  ...
  try {
    ...
    const res: any = await request.get('/erp/settlement-methods/page', { params });
...
const openEditModal = (row: SettlementMethod) => {
...
const saveData = async () => {
  ...
  try {
    ...
    const res: any = isEditing.value && currentId.value
      ? await request.put(`/erp/settlement-methods/${currentId.value}`, payload)
      : await request.post('/erp/settlement-methods', payload);
...
const handleDelete = async (row: SettlementMethod) => {
  try {
