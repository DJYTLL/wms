<template>
  <div class="page-shell page-shell--system">
    <div class="page-header">
      <div class="page-title">{{ $t('page.erpUnitManagement') }}</div>
      <div class="page-toolbar-card">
        <div class="erp-basic-toolbar">
          ...
          <div class="erp-basic-actions">
            <el-button type="primary" v-permission="'erp-unit:add'" @click="openAddModal">{{ $t('action.add') }}</el-button>
          </div>
        </div>
      </div>
    </div>
    ...
    <el-dialog v-model="showModal" :title="isEditing ? $t('action.edit') : $t('action.add')" width="640px" @closed="resetForm">
      <el-form :model="formData" label-width="120px">
        ...
        <el-form-item :label="$t('field.symbol')">
          <el-input v-model="formData.symbol" />
        </el-form-item>
        <el-form-item :label="$t('field.precision')">
          <el-input v-model="formData.precision" type="number" />
        </el-form-item>
...
const size = ref(20);
const total = ref(0);
const tableData = ref<ErpUnit[]>([]);
const showModal = ref(false);
const isEditing = ref(false);
const currentId = ref<number | null>(null);

const defaultColumns = ['code', 'name', 'symbol', 'precision', 'status'];
const { isVisible, fetchTenantKeys } = useColumnSettings('erp-unit', defaultColumns);
...
const fetchList = async () => {
  ...
  try {
    ...
    const res: any = await request.get('/erp/units/page', { params });
...
const openEditModal = (row: ErpUnit) => {
...
const saveData = async () => {
  ...
  try {
    ...
    const res: any = isEditing.value && currentId.value
      ? await request.put(`/erp/units/${currentId.value}`, payload)
      : await request.post('/erp/units', payload);
...
const handleDelete = async (row: ErpUnit) => {
  try {
    await request.delete(`/erp/units/${row.id}`);
    notifySuccess();
    fetchList();
  } catch (error) {
    notifyError(error);
