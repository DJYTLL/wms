...
const allTableData = ref<ErpUnit[]>([]);
...
const applySearch = () => {
  ...
  filtered = filterByFuzzyKeyword(filtered, symbolQuery.value, row => [row.symbol]);
...
const fetchNextCode = async () => {
  try {
    const res: any = await request.get('/erp/units/next-code');
    if (res.data.code === 200) {
...
const saveData = async () => {
  ...
  try {
    ...
    if (res.data.code === 200) {
      invalidateErpBaseDataResourceCache('units', tenantCacheKey.value);
      notifySuccess();
