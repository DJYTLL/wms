<template>
  <div class="print-page">
    <div v-if="!isPreview" class="print-toolbar">
      <div class="toolbar-title">{{ $t('page.erpPurchaseOrderPrint') }}</div>
...
const headerFieldLabel = (key: string) => {
  const mapping: Record<string, string> = {
    ...
    paidAmount: t('field.paymentAmount'),
...
const headerFieldValue = (key: string) => {
  ...
  switch (key) {
    ...
    case 'orderAt':
      return formatDateTime(current.createdAt);
    case 'supplierName':
      return getSupplierName(current.supplierId);
    case 'paymentMethod':
      return mapMethodName(paymentMethods.value, current.paymentMethodCode);
    case 'paidAmount':
...
const fetchTemplate = async () => {
  try {
    const res: any = await request.get('/erp/print-templates/default', { params: { docType: 'PURCHASE_ORDER' } });
