...
import { getCachedPaymentMethods } from '@/composables/erpBaseDataCache';
...
const fetchTemplate = async () => {
  try {
    ...
    template.value = await fetchPrintTemplate('PAYMENT', resolveTemplateId(route.query.templateId));
    applyTemplate(template.value);
  } catch {
    template.value = null;
    applyTemplate();
...
const fetchOptions = async () => {
  paymentMethods.value = await getCachedPaymentMethods(tenantCacheKey.value);
};
