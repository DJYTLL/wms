<template>
  <div class="page-shell page-shell--system">
    ...
    <el-dialog v-model="showModal" class="supplier-dialog" width="960px" top="6vh" @closed="handleDialogClosed">
      <template #header>
      ...
      <el-form :model="formData" label-position="top" class="supplier-dialog__form">
        <section class="supplier-section">
          ...
          <div class="supplier-section__grid">
            ...
            <el-form-item :label="$t('field.status')" class="supplier-section__span-2 supplier-status-panel__field">
              <div class="supplier-status-panel">
                <div class="supplier-status-panel__header">
                  <span class="supplier-status-panel__badge">{{ $t('field.status') }}</span>
                  <span class="supplier-status-panel__hint">{{ $t('supplierDialog.statusHint') }}</span>
                </div>
                <el-radio-group v-model="formData.status" class="supplier-status-panel__group">
                  <el-radio value="enabled" border>{{ $t('status.active') }}</el-radio>
                  <el-radio value="disabled" border>{{ $t('status.inactive') }}</el-radio>
                  <el-radio value="blacklisted" border>{{ $t('status.blacklisted') }}</el-radio>
                </el-radio-group>
              </div>
            </el-form-item>
          </div>
        ...
        <section class="supplier-section">
          ...
          <div class="supplier-section__grid">
            ...
            <el-form-item :label="$t('field.contactInfo')" class="supplier-section__span-2">
        ...
        <section class="supplier-section supplier-section--optional">
          <button
            ...
            :aria-expanded="optionalSectionsExpanded"
            aria-controls="supplier-optional-sections"
            @click="toggleOptionalSections"
          >
            <span class="supplier-section__toggle-copy">
              <span class="supplier-section__title">{{ $t('supplierDialog.sectionFinanceOptional') }}</span>
              <span class="supplier-section__toggle-hint">{{ $t('supplierDialog.optionalToggleHint') }}</span>
            </span>
            <span class="supplier-section__toggle-text">
              {{ optionalToggleLabel }}
            </span>
          </button>
          <div v-show="optionalSectionsExpanded" id="supplier-optional-sections" class="supplier-section__grid">
        ...
        <section class="supplier-section supplier-section--optional">
          <div class="supplier-section__header supplier-section__header--muted">
            <div class="supplier-section__title">{{ $t('supplierDialog.sectionHistoryOptional') }}</div>
            <div class="supplier-section__toggle-hint">{{ $t('supplierDialog.optionalSharedState') }}</div>
          </div>
          <div v-show="optionalSectionsExpanded" class="supplier-section__grid">
      ...
      <template #footer>
        <div class="supplier-dialog__footer">
          <el-button text class="supplier-dialog__footer-toggle" @click="toggleOptionalSections">
            {{ optionalToggleLabel }}
          </el-button>
...
import { computed, ref, reactive, onMounted, onActivated, nextTick } from 'vue';
...
const optionalSectionsExpanded = ref(false);
...
const optionalToggleLabel = computed(() => (
  optionalSectionsExpanded.value ? t('supplierDialog.collapseOptional') : t('supplierDialog.expandOptional')
));
...
const toggleOptionalSections = () => {
  optionalSectionsExpanded.value = !optionalSectionsExpanded.value;
};
...
const openEditModal = (row: ErpSupplier) => {
  ...
  showModal.value = true;
  optionalSectionsExpanded.value = false;
};
