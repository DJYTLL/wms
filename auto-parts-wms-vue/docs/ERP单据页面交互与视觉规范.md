...
- `sale-title-group` 必须保持横向排列：`page-title` 和 `sale-breadcrumb` 同行展示，业务模块不得用自定义类覆盖成纵向布局。
- `sale-breadcrumb` 必须 `inline-flex` 且 `white-space: nowrap`，避免在标题下方换行或与整体单据页风格不一致。
- `sale-header-card`、`sale-detail-card`、`payment-card` 必须有明确内边距，卡片内容不能直接贴住卡片边框。
- 基础信息区默认使用两列栅格；除非字段是长文本 textarea、地址类长字段或业务明确要求，否则普通输入项不要独占整行。
...
- 明细卡片内部必须保留统一内边距，表格和新增行按钮不能直接贴住 card 边缘。
- 明细标题和表格之间必须保留稳定间距，不能让标题条、表格边框、卡片边框三者贴在一起。
- `sale-detail-card .table-body` 如果继承列表页样式，应在单据页内重置为不额外包裹、不贴边的表单页布局。
- 新增行按钮应位于表格下方并与表格保持间距，左侧与表格内容区对齐。
...
| 标题面包屑 | `page-title + sale-breadcrumb` 横向同行，面包屑不换行 |
| 基础信息栅格 | 普通字段默认两列，长文本字段才允许跨整行 |
| 卡片内边距 | 基础信息、明细信息、结算信息卡片内容不得贴边 |
| 列表表格 | 草稿/已审核复用组件，通过 route meta 锁状态 |
| 明细表格 | 新增/编辑控件密集但稳定，查看页只读展示 |
| 明细留白 | 明细标题、表格、新增行按钮之间必须有稳定间距 |
