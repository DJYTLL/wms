-- Seed API latency monitor permission and menu for existing databases.
...
FROM (
    VALUES
        ('api-latency-monitor:view', '查看接口耗时查询', '查看前端接口耗时记录')
...
WHERE parent_menu.code = 'system'
  ...
  AND NOT EXISTS (
      ...
      WHERE existing.code = 'api-latency-monitor'
        AND existing.deleted_at IS NULL
  );
...
SET parent_id = parent_menu.id,
    title = '接口耗时查询',
    i18n_key = 'api-latency-monitor',
    path = '/api-latency-monitor',
    permission_code = 'api-latency-monitor:view',
    sort = 37,
    is_enabled = TRUE,
    updated_at = NOW()
FROM app_menu parent_menu
WHERE app_menu.code = 'api-latency-monitor'
...
JOIN app_menu menu
  ON menu.code = 'api-latency-monitor'
...
JOIN app_permission permission
  ON permission.code = 'api-latency-monitor:view'
