...
public class DatabaseUserServiceImpl implements UserAccountService {
    ...
    @Override
    public AuthPayload loadAuthPayload(String username, Long audienceTenantId) {
        AuthPayload payload = buildAuthPayload(loadAuthContext(username));
        if (audienceTenantId == null || audienceTenantId.equals(payload.tenantId())) {
            return payload;
        }
        if (payload.user() == null || payload.user().roles() == null || payload.user().roles().stream()
            .noneMatch(code -> code != null && "super_admin".equalsIgnoreCase(code))) {
        ...
        return new AuthPayload(
            ...
            payload.authVersion(),
            audienceTenantId,
            resolveTenantCode(audienceTenantId),
