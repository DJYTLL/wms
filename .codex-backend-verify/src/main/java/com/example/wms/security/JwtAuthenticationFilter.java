...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private UserDetails mergeTokenIdentityWithBackendAuthorities(UserDetails tokenDetails, UserDetails backendDetails) {
        if (!(tokenDetails instanceof AuthenticatedUser tokenUser)) {
            return backendDetails;
        }
        if (!(backendDetails instanceof AuthenticatedUser backendUser)) {
            return AuthenticatedUser.fromToken(
                tokenUser.getUserId(),
                tokenUser.getUsername(),
                tokenUser.getAuthPayload(),
                backendDetails.getAuthorities()
            );
        }
        return AuthenticatedUser.fromToken(
            tokenUser.getUserId(),
