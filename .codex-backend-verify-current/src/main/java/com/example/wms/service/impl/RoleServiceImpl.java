...
import com.example.wms.dto.RoleResponse;
...
public class RoleServiceImpl implements RoleService {
    ...
    public List<RoleResponse> listAll() {
        ...
        return roleMapper.selectList(new QueryWrapper<Role>()
            ...
            .filter(rolePolicyService::canViewRole)
            .map(this::toResponse)
            .toList();
    }

    @Override
    public PageResponse<RoleResponse> page(long page, long size, String keyword, Boolean enabled) {
        Long tenantId = TenantContext.requireTenantId();
        ...
        List<RoleResponse> items = visibleRoles.subList(fromIndex, toIndex).stream()
            .map(this::toResponse)
            .toList();
        return new PageResponse<>(visibleRoles.size(), safePage, safeSize, items);
    }
    ...
    public RoleResponse getById(Long id) {
        ...
        Role role = roleMapper.selectOne(new QueryWrapper<Role>()
            .eq("tenant_id", tenantId)
            .eq("id", id));
        if (!rolePolicyService.canViewRole(role)) {
            return null;
        }
    ...
    private void ensureManageableRole(Role role) {
        if (rolePolicyService.isCurrentActorRole(role.getId())) {
            throw new IllegalArgumentException("不能修改当前登录账号所属角色");
        }
        if (!rolePolicyService.canManageRole(role)) {
    ...
    private RoleResponse toResponse(Role role) {
        return new RoleResponse(
            ...
            role.getDescription(),
            role.isEnabled(),
            rolePolicyService.capabilities(role)
        );
    }
}
