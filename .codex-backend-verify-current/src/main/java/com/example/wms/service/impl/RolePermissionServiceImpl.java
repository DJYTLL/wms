...
public class RolePermissionServiceImpl implements RolePermissionService {
    ...
    private List<Permission> validateAndLoadPermissions(Role role, List<Long> permissionIds) {
        ...
        List<String> unassignable = permissions.stream()
            .filter(permission -> !roleScopeService.canAssignPermissionToRole(role, permission))
            .map(Permission::getCode)
            .filter(code -> code != null && !code.isBlank())
            .toList();
        if (!unassignable.isEmpty()) {
            throw new IllegalArgumentException("存在当前账号不可分配给该角色的权限: " + String.join("、", unassignable));
        }
        ensurePermissionsWithinActorScope(permissions);
        return permissions;
    }
