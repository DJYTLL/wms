...
#### 10.3.6 初始库存（INIT）补充规则

- 初始库存用于系统启用时的一次性建账，不用于日常库存修正。
- 初始库存明细必须提供 `initUnitCost` 或 `initTotalAmount`，系统会落库存成本快照并据此生成库存流水金额。
- 初始库存只能存在一张已审核单；创建和审核阶段都会做业务校验，数据库层也有唯一约束兜底。
- 老的初始库存单如果没有期初成本快照，不允许红冲。
- 初始库存单审核后，只要相关商品仓位已经发生后续库存流水，就不允许再红冲；如需修正，应改走库存调整单。
- 初始库存红冲必须填写原因，红冲备注中会保留原因用于审计。
