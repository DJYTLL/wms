CREATE TEMP TABLE tmp_customer_product_import_permission (
    code TEXT PRIMARY KEY,
    name TEXT NOT NULL,
    description TEXT
) ON COMMIT DROP;

INSERT INTO tmp_customer_product_import_permission (code, name, description)
VALUES
    ('erp-customer:import', '导入客户(ERP)', '导入ERP客户历史数据'),
    ('erp-product:import', '导入商品(ERP)', '导入ERP商品历史数据');
...
SET name = seed.name,
    ...
    updated_at = NOW(),
    deleted_at = NULL
FROM tmp_customer_product_import_permission seed
...
JOIN app_permission permission
  ON permission.code IN (SELECT code FROM tmp_customer_product_import_permission)
