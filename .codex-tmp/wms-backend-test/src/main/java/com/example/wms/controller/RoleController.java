...
public class RoleController {
    ...
    public ResponseEntity<ApiResponse<List<RoleOptionResponse>>> listOptions() {
        List<RoleOptionResponse> options = roleService.listAll().stream()
            .filter(RoleResponse::enabled)
            .filter(role -> role.capabilities() != null && role.capabilities().canManageColumnPermissions())
            .map(role -> new RoleOptionResponse(role.id(), role.code(), role.name()))
            .toList();
    ...
    @GetMapping("/page")
    @PreAuthorize("hasAuthority('PERM_role:view')")
    public ResponseEntity<ApiResponse<PageResponse<RoleResponse>>> page(@RequestParam(defaultValue = "1") long page,
