...
public class PermissionServiceImpl implements PermissionService {
    ...
    @Override
    public List<Permission> listAssignableToRole(String roleCode) {
        Role role = new Role();
        role.setCode(roleCode);
        return listAll().stream()
            .filter(permission -> rolePolicyService.canAssignPermissionToRole(role, permission))
            .toList();
    }
...
@Transactional
    ...
    public List<PermissionDiagnosticResponse> listDiagnostics() {
        ...
        Map<Long, Long> roleCounts = loadRoleCounts(permissions);
        Map<String, Long> menuCounts = loadMenuCounts(permissions);
        return permissions.stream()
            .map(permission -> toDiagnosticResponse(permission, roleCounts, menuCounts))
            .toList();
    }

    private PermissionDiagnosticResponse toDiagnosticResponse(Permission permission,
                                                              Map<Long, Long> roleCounts,
                                                              Map<String, Long> menuCounts) {
        long roleCount = roleCounts.getOrDefault(permission.getId(), 0L);
        long menuCount = permission.getCode() == null
            ? 0L
            : menuCounts.getOrDefault(permission.getCode(), 0L);
    ...
    private Map<Long, Long> loadRoleCounts(List<Permission> permissions) {
        ...
        Map<Long, RolePermissionMapper.RolePermissionCountRow> rows =
            rolePermissionMapper.countActiveRolesByPermissionIds(permissionIds);
        ...
        return rows.entrySet().stream()
            .collect(java.util.stream.Collectors.toMap(
                Map.Entry::getKey,
                entry -> {
                    Long count = entry.getValue() == null ? null : entry.getValue().getRoleCount();
                    return count == null ? 0L : count;
                }
            ));
    }

    private Map<String, Long> loadMenuCounts(List<Permission> permissions) {
        ...
        Map<String, MenuMapper.MenuPermissionCountRow> rows =
            menuMapper.countActiveMenusByPermissionCodes(permissionCodes);
        if (rows == null || rows.isEmpty()) {
            return Map.of();
        }
        return rows.entrySet().stream()
            .collect(java.util.stream.Collectors.toMap(
                Map.Entry::getKey,
                entry -> {
                    Long count = entry.getValue() == null ? null : entry.getValue().getMenuCount();
                    return count == null ? 0L : count;
                }
            ));
    }
